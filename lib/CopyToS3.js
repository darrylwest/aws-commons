...
var CopyToS3 = function(options) {
    ...
    this.readFileCallback = function(err, data) {
        ...
        if (!mime) {
            mime = copier.determineMimeType( key );
        }
        ...
        if (mime) {
            log.info('set the mime/content type: ', mime);
    ...
    this.determineMimeType = function(key) {
        var type;
        ...
        log.info('mime type determined: ', type);
